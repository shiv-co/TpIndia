...
export default function GetQuote({
  ...
  highlight = "Story?",
  subtitle = "Contact TP India Network and we will plan, shoot and deliver with precision, creativity, and care. Let's create something unforgettable.",
  buttonLabel = "Contact Us",
  to = "/contact",
  sectionClassName = "bg-[var(--bg-color)]",
}) {
  const titleParts = title.split(highlight);

  return (
    <section className={`relative py-24 overflow-hidden text-[var(--text-primary)] w-full ${sectionClassName}`}>
      <motion.div
      ...
      <div className="relative z-10 max-w-3xl mx-auto text-center px-6">
        ...
        >
          {titleParts[0]}
          <span className="text-[var(--accent-color)]">{highlight}</span>
          {titleParts[1] || ""}
        </motion.h3>
        ...
        >
          <Link
            to={to}
            className="px-8 py-3 bg-[var(--accent-color)] text-white rounded-full font-semibold shadow-lg hover:shadow-[0_0_20px_rgba(65,105,225,0.6)] hover:scale-105 transition-all"
          >
            {buttonLabel}
          </Link>
        </motion.div>
